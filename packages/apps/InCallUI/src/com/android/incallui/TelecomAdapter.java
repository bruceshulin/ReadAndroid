...
final class TelecomAdapter implements InCallPhoneListener {
    ...
    /** Identifier for the subId intent extra. */
    private static final String ADD_CALL_SUB_ID = "add_call_sub_id";
    /* @} */
    ...
    public void clearPhone() {
        /** SPRD: a work-around method for sockets leak problem.
         * Clear up P-sensor before phone cleared. @{ */
        InCallPresenter.getInstance().clearUpProximitySensor();
        /** @} */
        mPhone = null;
    }
    ...
    void addCall(int subId) {
        if (mContext != null) {
            ...
            // InCallScreen / PhoneUtils.
            if (mPhone != null) {
                intent.putExtra(ADD_CALL_SUB_ID, subId);
            }
            intent.putExtra(ADD_CALL_MODE_KEY, true);
            try {
                Log.d(this, "Sending the add Call intent");
