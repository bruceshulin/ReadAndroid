...
public class CallCardPresenter extends Presenter<CallCardPresenter.CallCardUi>
    ...
    public void onStateChange(InCallState oldState, InCallState newState, CallList callList) {
        ...
        int subId = -1; // SPRD: modify for bug 430163
        if (mPrimary != null) {
            callState = mPrimary.getState();
            if (mPrimary.getAccountHandle() != null && mPrimary.getAccountHandle().getId() != null && !(mPrimary.getAccountHandle().getId().equals("E"))) {
                subId = Integer.parseInt(mPrimary.getAccountHandle().getId()); // SPRD: modify for bug 430163
            }
    ...
    private void updatePrimaryCallState() {
        if (getUi() != null && mPrimary != null) {
            int subId = -1;
            if (mPrimary.getAccountHandle() != null && mPrimary.getAccountHandle().getId() != null && !(mPrimary.getAccountHandle().getId().equals("E"))) {
                /* SPRD: add try-catch for NumberFormatException bug451074 @{ */
                try {
                    subId = Integer.parseInt(mPrimary.getAccountHandle().getId()); // SPRD: modify for bug 430163
                } catch (NumberFormatException e) {
                    Log.e(this,"Exception raised during parsing int.",e);
                }
                /* @} */
            }
            getUi().setCallState(
                    subId, // SPRD: modify for bug 430163
                    mPrimary.getState(),
    ...
    private void setCallbackNumber() {
        ...
        boolean isEmergencyCall = PhoneNumberUtilsSprd.isEmergencyNumber(
    ...
    public void updateCallTime() {
        ...
        if (ui == null || mPrimary == null || mPrimary.getState() != Call.State.ACTIVE) {
            if (ui != null) {
                ui.setPrimaryCallElapsedTime(false, 0);
				Log.v("TimeBruce", "updateCallTime:----");
            }
            mCallTimer.cancel();
			Log.v("TimeBruce", "updateCallTime:22222");

        } else {
            final long callStart = mPrimary.getConnectTimeMillis();
            final long duration = System.currentTimeMillis() - callStart;
            ui.setPrimaryCallElapsedTime(true, duration);
			Log.v("TimeBruce", "updateCallTime:"+duration);
    ...
    public interface CallCardUi extends Ui {
        ...
        void setCallState(int subId,int state, int videoState, int sessionModificationState, // SPRD: modify for bug 430163
