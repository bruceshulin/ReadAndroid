...
public class CallList implements InCallPhoneListener {
    ...
    // SPRD: fix bug 432259
    private final HashMap<String, CallTemp> mCallTemp = new HashMap<>();
    /**
    ...
    public Call getFirstCall() {
        ...
        // SPRD BUG427275 resume InCallActivity while call state is on hold,return call is null
        if (result == null) {
    ...
    private boolean updateCallInMap(Call call) {
        ...
        /* SPRD: fix bug 432259 @{ */
        addVideoStateChangeCall(call);
        CallTemp callTemp = new CallTemp(call.getId(), call.getVideoState());
        mCallTemp.put(call.getId(), callTemp);
        /* @} */
        ...
        if (call.getState() == Call.State.DISCONNECTED) {
            ...
            // SPRD: SPRD: fix bug 432259
            mCallTemp.remove(call.getId());
            updated = true;
        }
    ...
    /* SPRD: fix bug 432259 @{ */
    private class CallTemp {
        private String mId;
        private int mVideoState;

        public CallTemp(String id, int videoState) {
            mId = id;
    ...
    private void addVideoStateChangeCall(Call call) {
        if (call == null) {
            return;
        }

        CallTemp existedCall = mCallTemp.get(call.getId());

        if ((existedCall != null) && (call.getId().equals(existedCall.getId()))
                && (existedCall.getVideoState() != VideoProfile.VideoState.AUDIO_ONLY)
                && (existedCall.getVideoState() != call.getVideoState())) {
            call.setVideoStateChange(true);
        }
    }
    /* @} */
    ...
    public boolean hasValidGroupCall() {
        Call multiCall = null;
        for (Call call : mCallById.values()) {
            if(call.isGroupCall() && call.getState() != Call.State.INVALID
