...
public class InCallPresenter implements CallList.Listener, InCallPhoneListener {
    ...
    // SPRD: fix bug 431749
    private boolean mShowAnswerUi = false;
    // SPRD: Add for recorder.
    private PhoneRecorderHelper mRecorderHelper;

    private int mVibrationDuration = 200;
    /* SPRD: fix bug 431749 @{ */
    public void setShowAnswerUi(boolean showUi) {
        mShowAnswerUi = showUi;
    }

    public boolean getShowAnswerUi() {
        return mShowAnswerUi;
    }
    /* @} */
    ...
    public void setUp(Context context, CallList callList, AudioModeProvider audioModeProvider) {
        ...
        //SPRD: bug439556 remove the original one before add new listener
    ...
    public void setIsRejectCallBySms(boolean isReject) {
        if (mInCallActivity != null) {
            mInCallActivity.setIsRejectCallBySms(isReject);
        }
    }

    public boolean getIsRejectCallBySms() {
        if (mInCallActivity != null) {
            return mInCallActivity.getIsRejectCallBySms();
        }
    ...
    public void onIncomingCall(Call call) {
        // SPRD: fix bug 431749
        setShowAnswerUi(false);
        // SPRD: fix bug 432115
        setIsRejectCallBySms(false);
    ...
    public void onDisconnect(Call call) {
        ...
        InCallState oldState = mInCallState;
	if(mInCallState != InCallState.WAITING_FOR_ACCOUNT){
	...
	if (isActivityStarted() && (oldState == InCallState.INCOMING && newState != InCallState.INCOMING)) {
    ...
    private void maybeShowErrorDialogOnDisconnect(Call call) {
        ...
        if (isActivityStarted() && call.getState() == Call.State.DISCONNECTED) {
            ...
            /* SPRD: fix bug 427540 @{*/
            if (PhoneNumberUtils.isEmergencyNumber(call.getNumber()) || PhoneNumberUtilsSprd.isCustomEmergencyNumber2(call.getNumber())) {
                String strOutofService  = mContext.getString(R.string.incall_error_out_of_service);
                CharSequence strCause = call.getDisconnectCause().getDescription();

                if (!TextUtils.isEmpty(strCause) && !strOutofService.equalsIgnoreCase(strCause.toString())) {
    ...
    private void setDisconnectCauseForMissingAccounts(Call call) {
        ...
        if (phoneAccountHandles == null || phoneAccountHandles.isEmpty()) {
            /* SPRD: modify for bug432875 {@ */
            String scheme = null;
            try {
                scheme = telecomCall.getDetails().getHandle().getScheme();
            } catch (NullPointerException ex) {
                Log.e(this, "NullPointerException occurs");
                ex.printStackTrace();
            }
            /* @} */
...
// =============================================================================
    ...
    public float getSpaceBesideCallCard2() {
        return mInCallActivity.getCallCardFragment().getSpaceBesideCallCard2();
    }
    ...
    /**
     * Proximity Sensor tears down when needed.
     */
    public void clearUpProximitySensor() {
        if (mProximitySensor != null) {
            removeListener(mProximitySensor);
            mProximitySensor.tearDown();
        }
